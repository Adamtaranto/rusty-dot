...
* **Re-using a name silently overwrites.** If `add_sequence` or a FASTA record uses a name that already exists in the index, the existing entry (and its FM-index) is replaced with a new one for the new sequence. No warning is raised.
...
# Re-using a name silently replaces the previous entry
