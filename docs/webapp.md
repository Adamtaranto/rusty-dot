...
| Alignment — via biowasm | **minimap2 2.22** (preset `asm20` by default) and **nucmer (MUMmer4)** (`-l 100 -c 200` by default) run in-browser via the [biowasm](https://biowasm.com) CDN — assembly-scale defaults; the tool binaries are fetched from biowasm.com at runtime, so these need a network connection. Runs can be cancelled mid-flight (switch method or re-run), and each run's exact command line and stderr appear in the collapsible **Aligner log**. |
